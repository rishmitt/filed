...
def create_api():
    if request.method == "POST":
        ...
        if type == "podcast":
            participant = metadata.get("participants", None)
            if (
                    participant is None
                    or len(participant) > 10
                    or any(i for i in participant if len(i) > 100)
            ):
                return "The request is invalid: 400 bad request", 400
        try:
...
@blueprint.route("/api/v1/update/<audioFileType>/<audioFileID>",
                 methods=["PUT"])
...
@blueprint.route("/api/v1/delete/<audioFileType>/<audioFileID>",
                 methods=["DELETE"])
...
@blueprint.route(
    "/api/v1/get/<audioFileType>", methods=["GET"],
    defaults={"audioFileID": None}
)
